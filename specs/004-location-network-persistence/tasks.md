...
- [x] T008 [US1] Add `private readonly List<Location> _knownLocationsSnapshot = new();` to `LocationService` and expose `IReadOnlyList<Location> GetAllKnownLocations()` returning `_knownLocationsSnapshot` (not `_locationCache.Values` — snapshot used for index access); rebuild `_knownLocationsSnapshot` on every cache mutation (add in `GetOrCreateLocation`, remove in `ForgetNetwork`/`ForgetDevice`); add unit test in `Assets/Tests/EditMode/LocationServiceTests.cs`: restore from 3-location save → `GetAllKnownLocations()` returns 3; verify same reference returned on second call (no allocation) — in `Assets/Scripts/Services/LocationService.cs`
...
- [x] T022 [US2] Add no-location error to `ScanCommand`: at start of no-args `scan` handler, if `HasCurrentLocation()` returns false (equivalent to empty `_locationCache`) return `CommandResult.Fail("No location available. Use 'move' to discover a location first.")`; add unit tests: no current location → failure result containing "move"; location present → normal scan output — in `Assets/Scripts/Services/Commands/ScanCommand.cs`
